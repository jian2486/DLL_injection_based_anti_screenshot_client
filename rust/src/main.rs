    ...
    fn show_mode_list(&mut self, ui: &mut egui::Ui, mut items: Vec<ListItem>, mode: i32, process_infos: &[loader::ProcessInfo]) {
        ...
        ui.vertical(|ui| {
            for (idx, item) in items.iter_mut().enumerate() {
                ...
                let matched_info = process_infos.iter().find(|proc_info| {
                    proc_info.name == process_name.as_str() && proc_info.path == process_path.as_str()
                });
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("清空").clicked() {
                    let pids_to_uninject: Vec<u32> = items.iter()
                        .flat_map(|item| {
                            process_infos.iter()
                                .filter(|proc_info| proc_info.name == item.text && proc_info.path == item.process_path)
        ...
        for (idx, name, path) in items_to_remove.into_iter() {
            let pids: Vec<u32> = process_infos.iter()
                .filter(|proc_info| proc_info.name == name && proc_info.path == path)
                .flat_map(|proc_info| proc_info.pids.clone())
                .collect();
        ...
        if let Some((item_name, item_path, enabled)) = toggled_item {
            let pids: Vec<u32> = process_infos.iter()
                .filter(|proc_info| proc_info.name == item_name && proc_info.path == item_path)
                .flat_map(|proc_info| proc_info.pids.clone())
                .collect();
        ...
        std::thread::spawn(move || {
            ...
            for item in &items_clone {
                ...
                for proc_info in &process_infos {
                    if proc_info.name == *process_name && (process_path.is_empty() || proc_info.path == *process_path) {
